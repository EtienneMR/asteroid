...
class SpaceShip(BaseEntity):
    ...
    def __init__(self, god_time: float):
        """
    ...
    def tick(self, deltaTime: float):
        ...
        self.velocity += self.orientation * (
            self.acceleration_input * self.LIN_ACCELERATION * deltaTime
        )

        self.ang_velocity += self.rotate_input * self.ANG_ACCELERATION * deltaTime
        self.ang_velocity *= 1 - self.ANG_FRICTION * deltaTime * (
            1 + abs(self.ang_velocity) / self.ANG_FRICTION_SCALE
        )
